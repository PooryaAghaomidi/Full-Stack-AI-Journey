...
* `git branch -a`: list all branches and master in local and remote repositories (master/main is the primary branch of a repository)
...
* `git remote add origin <GitHub URL>`: connect the local repository to the remote one on GitHub (origin refers to the remote repository)
* `git push origin main`: transfer the main branch of the local repository to the remote one on GitHub (origin/main refers to the main branch of the remote repository)
* `git fetch origin`: get the updates of the remote repository
* `git diff <branch name>`: display changes of a branch considering the current branch (we assume that origin/main is a branch)
* `git pull origin`: fetch + merge (get all changes and commits from the remote repository)
* `In GitHub you can submit a pull request to compare and merge your branches into the main branch (no code required)`
...
* `if the cloned repository is not ours, we are not allowed to make changes and we'd better to rename it from origin to upstream`
...
* `after changing the repository, we can push the changes to our fork and then submit a pull request to the main remote repository in GitHub (no code required)`


## Section Three: Undo
* `git revert HEAD~<the number of earlier commit>`: take a previous commit and add it as a new commit, keeping the log intact
